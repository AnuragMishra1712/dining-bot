...
def validate_booking(location, cuisine_type, booking_time, no_people, phone):
    if location is not None and location != "Manhattan":
        return build_validation_result(False, 'location', "Please enter a valid location")

    cuisine_types = ['chinese', 'indian', 'mexican', 'thai', 'american', 'italian']
    ...
    if booking_time is not None:
        ...
        if hour < 12 or hour > 23:
            ...
            return build_validation_result(False, 'BookingTime',
                                           'You can make a reservation from 12 noon to 11 p m. Can you specify a time during this range?')

    if no_people is not None:
        no_people = int(no_people)
        if no_people > 20:
            return build_validation_result(False, 'no_people',
                                           "You can make a reservation for 1 person and upto 20 people, for more than 20 please contact the restaurant directly. Can you specify a number in the range?  ")
...
def diningsuggestion_intent(intent_request):
    ...
    if source == 'DialogCodeHook':
        ...
        validation_result = validate_booking(location, cuisine_type, booking_time, no_people, phone)
    ...
    # queue = sqs.get_queue(QueueName='sqsq')
    msg = {'location': location, "cuisine": cuisine_type, "phone": phone, "email": Email}
    response = sqs.send_message(QueueUrl='https://sqs.us-east-1.amazonaws.com/658960012993/sqsq',
